...
class Trace(PDR):
    ...
    def set_whole_ukf(self):
        def fx(x, dt, X_a, Y_a, Z_a):
            Rm = R.from_rotvec(np.array([x[10], x[12], x[14]]))  # type: R

            X_a = X_a @ Rm.as_matrix()
            Y_a = Y_a @ Rm.as_matrix()
            Z_a = Z_a @ Rm.as_matrix()
    ...
    def filtered_data_whole(self,
        ...
        for i, row in self.df.iterrows():
            ...
            values = dict_prev_matrix.values()
            for val in values:
                vector = (self.ukf_whole.x[9],
                          self.ukf_whole.x[11],
                          self.ukf_whole.x[13])
                val *= R.from_rotvec(vector).as_matrix()
...
if __name__ == '__main__':
    ...
    trace.draw_plot(y_data=df_acc['f_p_X'],
                    x_data=df_acc['f_p_Y'],
                    show=True,
                    marker='^')
    print(df)
